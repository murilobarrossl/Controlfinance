...
import { getBankAccounts, createBankAccount, updateBankAccount, deleteBankAccount } from "../../api/finance.js";
import "./contas.css";
...
const getBankColor = (code) => BANKS.find((b) => b.code === code)?.color ?? "#8a9bbf";
const getBankName  = (code) => BANKS.find((b) => b.code === code)?.name  ?? "Banco";
...
export default function ContasBancarias() {
  ...
  const openEdit = (acc) => {
    setSelected(acc);
    setForm({ name: acc.name, bankCode: acc.bankCode ?? "001", balance: String(acc.balance) });
    setError("");
    setModal("edit");
  };
  ...
  const handleSave = async () => {
    ...
    if (isNaN(balance)) { setError("Saldo inválido."); return; }
    ...
    <div className="contas-root">
      ...
      <nav className="dash-nav">
        <a href="/dashboard" className="dash-nav-logo">
          <svg width="34" height="34" viewBox="0 0 34 34" fill="none">
            ...
            <path d="M6 19 L12 14 L18 9 L24 4" stroke="#e84520" strokeWidth="2" strokeLinecap="round"/>
        ...
        <div className="dash-nav-links">
          {NAV_LINKS.map((l) => (
            <Link key={l.to} to={l.to} className={l.to === "/contas-bancarias" ? "active" : ""}>
              {l.label}
            </Link>
          ))}
        </div>
      ...
      <div className="contas-body">
        ...
        <div className="contas-header">
          ...
          <button className="btn-primary" onClick={openCreate}>+ Nova Conta</button>
        </div>
        ...
        <div className="contas-total-card">
          ...
          <div className="contas-total-count">{accounts.length} conta{accounts.length !== 1 ? "s" : ""}</div>
        </div>
        ...
        {loading ? (
          ...
          <div className="contas-empty">
            ...
            <button className="btn-primary" onClick={openCreate}>Adicionar primeira conta</button>
          </div>
        ) : (
            ...
            {accounts.map((acc) => (
              <div key={acc.id} className="conta-card">
                <div className="conta-card-top">
                  <div className="conta-bank-icon" style={{ background: getBankColor(acc.bankCode) }}>
                    {(acc.bankCode ? getBankName(acc.bankCode) : acc.name).slice(0, 2).toUpperCase()}
                  </div>
                  <div className="conta-actions">
                    <button className="conta-btn-edit" onClick={() => openEdit(acc)} title="Editar">✎</button>
                    <button className="conta-btn-delete" onClick={() => openDelete(acc)} title="Excluir">✕</button>
                  </div>
                </div>
                <div className="conta-name">{acc.name}</div>
                <div className="conta-bank">{acc.bankCode ? getBankName(acc.bankCode) : "—"}</div>
                <div className="conta-balance">{fmt(acc.balance)}</div>
                <div className="conta-status">{acc.isActive ? "● Ativa" : "○ Inativa"}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {(modal === "create" || modal === "edit") && (
        <Modal title={modal === "create" ? "Nova Conta" : "Editar Conta"} onClose={() => setModal(null)}>
        ...
        <Modal title="Excluir Conta" onClose={() => setModal(null)}>
          <div className="modal-body">
            <p style={{ color: "#001647", marginBottom: 20 }}>
              Tem certeza que deseja excluir a conta <strong>{selected?.name}</strong>? Esta ação não pode ser desfeita.
            </p>
            <div className="modal-actions">
              <button className="btn-secondary" onClick={() => setModal(null)}>Cancelar</button>
